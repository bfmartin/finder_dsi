...
class String
  ...
  def stemmed
    return self if self == '\N' or self == 'NULL' # don't stem this
    self.split(/[\s\.\?\!,-]/).collect { |word| word.stem }.join(" ").gsub(/\s+/, ' ')
  end
...
class Date
  ...
  def self.parse_json(stringdate)
    Date.civil(stringdate[0,4].to_i, stringdate[5,2].to_i, stringdate[8,2].to_i)
  end
end
